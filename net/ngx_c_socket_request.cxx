...
ssize_t CSocekt::recvproc(lpngx_connection_t pConn,char *buff,ssize_t buflen)  //ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    if(n == 0)
    {
        //客户端关闭【应该是正常完成了4次挥手】，这边就直接回收连接，关闭socket即可 
    ...
    if(n < 0) //这被认为有错误发生
    {
        ...
        if(errno == EAGAIN || errno == EWOULDBLOCK)
        {
            ...
            ngx_log_stderr(errno,"CSocekt::recvproc()中errno == EAGAIN || errno == EWOULDBLOCK成立，出乎意料！");//epoll为LT模式不应该出现这个返回值，所以直接打印出来瞧瞧
            return -1; //不当做错误处理，只是简单返回
        }
        //EINTR错误的产生：当阻塞于某个慢系统调用的一个进程捕获某个信号且相应信号处理函数返回时，该系统调用可能返回一个EINTR错误。
        //例如：在socket服务器端，设置了信号捕获机制，有子进程，当在父进程阻塞于慢系统调用时由父进程捕获到了一个有效信号时，内核会致使accept返回一个EINTR错误(被中断的系统调用)。
        if(errno == EINTR)  //这个不算错误，是参考官方nginx，官方nginx这个就不算错误；
        {
            //LT模式不该出现这个errno，而且这个其实也不是错误，所以不当做错误处理
            ngx_log_stderr(errno,"CSocekt::recvproc()中errno == EINTR成立，出乎意料！");//epoll为LT模式不应该出现这个返回值，所以直接打印出来瞧瞧
            return -1; //不当做错误处理，只是简单返回
        }
        ...
        {
            //能走到这里的，都表示错误，打印一下日志，希望知道一下是啥错误，准备打印到屏幕上
            ...
            {
                ngx_log_stderr(errno,"CSocekt::recvproc()中发生错误，打印出来看看是啥错误！");  //正式运营时可以考虑这些日志打印去掉
            }
        } 
...
ssize_t CSocekt::sendproc(lpngx_connection_t c,char *buff,ssize_t size)  //ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    for ( ;; )
    {
        ...
        if(n == 0)
        {
            //send()返回0？ 一般recv()返回0表示断开,send()返回0，这里就直接返回0吧【让调用者处理】；个人认为send()返回0，要么你发送的字节是0，要么对端可能断开。
            //网上找资料：send=0表示超时，对方主动关闭了连接过程
            //写代码要遵循一个原则，连接断开，并不在send动作里处理诸如关闭socket这种动作，集中到recv那里处理，否则send,recv都处理都处理连接断开关闭socket则会乱套
        ...
        {
            //走到这里表示是其他错误码，都表示错误，错误也不断开socket，也依然等待recv()来统一处理断开，因为是多线程，send()也处理断开，recv()也处理断开，很难处理好
            return -2;    
        }
    } //end for
}
...
{      
    ...
    {
        //这不太可能，可以发送数据时通知发送数据，发送时你却通知发送缓冲区满？
    ...
    if(sendsize > 0 && sendsize == pConn->isendlen) //成功发送完毕，做个通知是可以的；
    {
        ...
        if(ngx_epoll_oper_event(
                pConn->fd,          //socket句柄
                EPOLL_CTL_MOD,      //事件类型，这里是修改【因为准备减去写通知】
                EPOLLOUT,           //标志，这里代表要减去的标志,EPOLLOUT：可写【可写的时候通知】
