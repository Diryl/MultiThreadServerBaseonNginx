...
//从连接池中获取一个空闲连接
//【当一个客户端连接TCP进入，希望把这个连接和连接池中的一个连接【对象】绑到一起，后续可以通过这个连接，把这个对象拿到，因为对象里边可以记录各种信息】
...
//有些连接，不希望马上释放，要隔一段时间后再释放以确保服务器的稳定，所以，把这种隔一段时间才释放的连接先放到一个队列中来
...
lblRRTD:
            ...
            for(; pos != posend; ++pos)
            {
                ...
                //到释放的时间了: 
                //......这将来可能还要做一些是否能释放的判断[在写完发送数据代码之后吧]，先预留位置
                //....

                //认为，凡是到释放时间的，iThrowsendCount都应该为0
...
//用户连入，accept4()时，得到的socket在处理中产生失败，则资源用这个函数释放【因为这里涉及到好几个要释放的资源，所以写成函数】
