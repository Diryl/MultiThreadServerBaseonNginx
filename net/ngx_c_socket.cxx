...
void CSocekt::Shutdown_subproc()
{
    ...
    if((&m_semEventSendQueue)==-1)  //让ServerSendQueueThread()流程走下来干活
    {
...
{   
    //等待事件，事件会返回到m_events里，最多返回NGX_MAX_EVENTS个事件【因为提供了这些内存】；
    ...
    for(int i = 0; i < events; ++i)    //遍历本次epoll_wait返回的所有事件，注意events才是返回的实际事件数量
    {
        ...
        {                 
            //比如用epoll_wait取得三个事件，处理第一个事件时，因为业务需要，把这个连接关闭，那应该会把c->fd设置为-1；
            //第二个事件照常处理
        ...
        if(c->instance != instance)
        {
            ...
            //比如用epoll_wait取得三个事件，处理第一个事件时，因为业务需要，把这个连接关闭【麻烦就麻烦在这个连接被服务器关闭上了】，但是恰好第三个事件也跟这个连接有关；
        ...
        */
        ...
        if(revents & EPOLLOUT) //如果是写事件【对方关闭连接也触发这个，再研究。。。。。。】，注意上边的 if(revents & (EPOLLERR|EPOLLHUP))  revents |= EPOLLIN|EPOLLOUT; 读写标记都给加上了
        {
            ...
            if(revents & (EPOLLERR | EPOLLHUP | EPOLLRDHUP)) //客户端关闭，如果服务器端挂着一个写通知事件，则这里个条件是可能成立的
            {
                ...
                //只有投递了 写事件，但对端断开时，程序流程才走到这里，投递了写事件意味着 iThrowsendCount标记肯定被+1了，这里减回
...
{    
    ...
    while(g_stopEvent == 0) //不退出
    {
        ...
        {
            //失败？及时报告，其他的也不好干啥
            if(errno != EINTR) //这个就不算个错误了【当阻塞于某个慢系统调用的一个进程捕获某个信号且相应信号处理函数返回时，该系统调用可能返回一个EINTR错误。】
        ...
        {
            err = pthread_mutex_lock(&pSocketObj->m_sendMessageQueueMutex); //因为要操作发送消息对列m_MsgSendQueue，所以这里要临界            
                ...
                {                    
                    ...
                    {                        
                        ...
                        if(pSocketObj->ngx_epoll_oper_event(
                                ...
                                EPOLLOUT,           //标志，这里代表要增加的标志,EPOLLOUT：可写【可写的时候通知】
