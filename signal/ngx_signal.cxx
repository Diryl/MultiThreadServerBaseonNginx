...
typedef struct 
{
    ...
    //信号处理函数,这个函数由自己来提供，但是它的参数和返回值是固定的【操作系统就这样要求】,大家写的时候就先这么写，也不用思考这么多；
...
ngx_signal_t  signals[] = {
    ...
    { SIGSYS,    "SIGSYS, SIG_IGN",  NULL               },        //想忽略这个信号，SIGSYS表示收到了一个无效系统调用，如果不忽略，进程会被操作系统杀死，--标识31
                                                                  //所以把handler设置为NULL，代表 要求忽略这个信号，请求操作系统不要执行缺省的该信号处理动作（杀掉）
    //...日后根据需要再继续增加
...
int ngx_init_signals()
{
    ...
    {        
        //注意，现在要把一堆信息往 变量sa对应的结构里弄 ......
        memset(&sa,0,sizeof(struct sigaction));

        if (sig->handler)  //如果信号处理函数不为空，这当然表示要定义自己的信号处理函数
        {
        ...
        else
        {
            sa.sa_handler = SIG_IGN; //sa_handler:这个标记SIG_IGN给到sa_handler成员，表示忽略信号的处理程序，否则操作系统的缺省信号处理程序很可能把这个进程杀掉；
                                      //其实sa_handler和sa_sigaction都是一个函数指针用来表示信号处理程序。只不过这两个函数指针参数不一样， sa_sigaction带的参数多，信息量大，
                                       //而sa_handler带的参数少，信息量少；如果你想用sa_sigaction，那么你就需要把sa_flags设置为SA_SIGINFO；                                       
        } //end if

        sigemptyset(&sa.sa_mask);   //比如处理某个信号比如SIGUSR1信号时不希望收到SIGUSR2信号，那就可以用诸如sigaddset(&sa.sa_mask,SIGUSR2);这样的语句针对信号为SIGUSR1时做处理；
                                    //这里.sa_mask是个信号集（描述信号的集合），用于表示要阻塞的信号，sigemptyset()这个函数：把信号集中的所有信号清0，本意就是不准备阻塞任何信号；
                                    
        
        //设置信号处理动作(信号处理函数)，说白了这里就是让这个信号来了后调用的处理程序，有个老的同类函数叫signal，不过signal这个函数被认为是不可靠信号语义，不建议使用，大家统一用sigaction
...
static void ngx_process_get_status(void)
{
    ...
    for ( ;; ) 
    {
        ...
        //-------------------------------
        if(pid == -1)//这表示这个waitpid调用有错误，有错误也理解返回出去，管不了这么多
        {
