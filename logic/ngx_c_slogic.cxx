...
{          
    ...
    //要做一些判断
    //(1)如果从收到客户端发送来的包，到服务器释放一个线程池中的线程处理该包的过程中，客户端断开了，那显然，这种收到的包就不必处理了；    
...
{    
    ...
    p_RecvInfo->username[sizeof(p_RecvInfo->username)-1]=0; //防止恶意用户
    p_RecvInfo->password[sizeof(p_RecvInfo->password)-1]=0; //防止恶意用户
    ...
    char *p_sendbuf = (char *)p_memory->AllocMemory(m_iLenMsgHeader+m_iLenPkgHeader+iSendLen,false); //申请发送的消息包内存 
    memcpy(p_sendbuf,pMsgHeader,m_iLenMsgHeader);                   //消息头直接拷贝，包括该连接信息+连接序号
    pPkgHeader = (LPCOMM_PKG_HEADER)(p_sendbuf+m_iLenMsgHeader);    //包头指针
    pPkgHeader->msgCode = _CMD_LOGIN;                               //包头信息：消息代号
    pPkgHeader->msgCode = htons(pPkgHeader->msgCode);               //unsiged short int, 网络传输：多字节需要转为网络字节序
    pPkgHeader->pkgLen  = htons(m_iLenPkgHeader + iSendLen);        //包头信息：包头+包体长度
    LPSTRUCT_LOGIN p_sendInfo = (LPSTRUCT_LOGIN)(p_sendbuf+m_iLenMsgHeader+m_iLenPkgHeader); //包体指针
    pPkgHeader->crc32   = p_crc32->Get_CRC((unsigned char *)p_sendInfo,iSendLen); //包头信息：包体检验信息
    pPkgHeader->crc32   = htonl(pPkgHeader->crc32);	                // 多字节转化为网络字节序
