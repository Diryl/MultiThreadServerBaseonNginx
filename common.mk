...
#要把目标文件生成到上述目标文件目录去，利用函数addprefix增加个前缀
...
#因为构建依赖关系时app目录下这个.o文件还没构建出来，所以LINK_OBJ是缺少这个.o的，要把这个.o文件加进来
LINK_OBJ += $(OBJS)
...
#做这个的最终目的说白了是，即便.h被修改了，也要让make重新编译的工程，否则，你修改了.h，make不会重新编译，那不行的
...
#----------------------------------------------------------------3begin-----------------
#现在希望当修改一个.h时，也能够让make自动重新编译的项目，所以，需要指明让.o依赖于.h文件
#那一个.o依赖于哪些.h文件，可以用“gcc -MM c程序文件名” 来获得这些依赖信息并重定向保存到.d文件中
#.d文件中的内容可能形如：nginx.o: nginx.c ngx_func.h
#%.d:%.c
$(DEP_DIR)/%.d:%.cxx
#gcc -MM $^ > $@
#.d文件中的内容形如：nginx.o: nginx.c ngx_func.h ../signal/ngx_signal.h，但现在的问题是.o文件已经放到了专门的目录
# 所以要正确指明.o文件路径这样，对应的.h,.c修改后，make时才能发现，这里需要用到sed文本处理工具和一些正则表达式语法，不必深究
