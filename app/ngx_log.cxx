...
{    
    ...
    u_char  errstr[NGX_MAX_ERROR_STR+1]; //2048  -- ************  +1是自己填的，感谢官方写法有点小瑕疵，所以动手调整一下
    u_char  *p,*last;
    ...
    last = errstr + NGX_MAX_ERROR_STR;        //last指向整个buffer最后去了【指向最后一个有效位置的后面也就是非有效位】，作为一个标记，防止输出内容超过这么长,
                                                    //其实这有问题，所以才在上边errstr[NGX_MAX_ERROR_STR+1]; 给加了1
    ...
    if (p >= (last - 1))
    {
        p = (last - 1) - 1; //把尾部空格留出来，这里感觉nginx处理的似乎就不对 
                             //last-1，才是最后 一个而有效的内存，而这个位置要保存\0，所以再减1，这个位置，才适合保存\n
    }
    *p++ = '\n'; //增加个换行符    
    
    //往标准错误【一般是屏幕】输出信息    
    write(STDERR_FILENO,errstr,p - errstr); //STDERR_FILENO这个叫标准错误，一般指屏幕
...
u_char *ngx_log_errno(u_char *buf, u_char *last, int err)
{
    ...
    {
        //保证整个装得下，就装，否则全部抛弃 ,nginx的做法是 如果位置不够，就硬留出50个位置【哪怕覆盖掉以往的有效内容】，也要硬往后边塞，这样当然也可以；
...
{
    u_char  *last;
    u_char  errstr[NGX_MAX_ERROR_STR+1];   //这个+1也是放入进来的，本函数可以参考ngx_log_stderr()函数的写法；
